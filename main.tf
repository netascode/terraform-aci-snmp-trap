...
resource "aci_rest" "snmpTrapDest" {
  ...
  dn         = "${aci_rest.snmpGroup.dn}/trapdest-${each.value.hostname_ip}-port-${each.value.port != null ? each.value.port : 162}"
...
resource "aci_rest" "fileRsARemoteHostToEpg" {
  ...
  dn         = "${aci_rest.snmpTrapDest[each.value.hostname_ip].dn}/rsARemoteHostToEpg"
